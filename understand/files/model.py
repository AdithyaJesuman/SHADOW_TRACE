...
class CardDB(Base):
    ...
    card_fingerprint: Mapped[str] = mapped_column(String(50), unique=True, nullable=False)  # uq_cards_fingerprint
...
# ---- transactions (FK: customer_id*, merchant_id, card_id, device_id, ip_id) ----

class TransactionDB(Base):
    __tablename__ = "transactions"
    __table_args__ = (
        # partial unique index — same as uq_tx_idempotency_key in ps.py,
        # NULLs allowed so ad-hoc inserts without a key still work
        Index("uq_tx_idempotency_key", "idempotency_key", unique=True,
...
# ---- transaction_features (FK: transaction_id*, one-to-one) --------------------
...
# ---- customer_identity (FK: transaction_id*, customer_id*) ---------------------
...
# ---- login_events (FK: customer_id*, ip_id, device_id) -------------------------
